...
class BrNuccChunkClump(BrNuccChunk):
    def init_data(self, br: BinaryReader):
        ...
        if self.field00 == 2:
            self.bounding_box = br.read_float(6)  # Bounding box
...
class BrNuccChunkModel(BrNuccChunk):
    def init_data(self, br: BinaryReader):
        ...
        self.nud = br.read_bytes(self.nudSize)
    ...
    def __br_write__(self, br: 'BinaryReader', chunkIndexDict: IterativeDict):
        ...
        br.write_uint32(len(self.nuccChunk.nud))
        br.write_float(self.nuccChunk.bounding_box)

        br.write_bytes(self.nuccChunk.nud)
...
class BrNuccChunkModelHit(BrNuccChunk):
    def init_data(self, br: BinaryReader):
        ...
        self.nucc_version = self.version
    ...
    def __br_write__(self, br: 'BinaryReader', chunkIndexDict: IterativeDict):
        ...
        for hit in self.nuccChunk.vertex_sections:
            br.write_struct(BrModelHit(), hit, self.nuccChunk.nucc_version)
...
class BrNuccChunkPrimitiveVertex(BrNuccChunk):
    def init_data(self, br: BinaryReader):
        ...
        self.vertices = br.read_struct(BrPrimitiveVertex, self.vertex_count)


class BrPrimitiveVertex(BrStruct):
    def __br_read__(self, br: BinaryReader):
        self.position = br.read_float(3)
        br.seek(4, 1)
